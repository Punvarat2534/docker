...
class book_controller{
    ...
    search = async function (req, res, next) {
       ...
       if(page!=0){
        ...
        //or title like %$1% or author like %$1% or publisher like %$1%
        var sql = 'SELECT * from public."book_detail" where isbn like %;$1% and deleted_at is null order by id desc limit 10 offset $2';
        const result = await Promise.all([
           
        //client.query('SELECT * from public."book_detail" where (isbn like %$1% or title like %$1% or author like %$1% or publish like %$1%)  and deleted_at is null order by id desc limit 10 offset $1',[page])

        //client.query('SELECT * from public."book_detail" where (isbn like %$1% or title like %$1% or author like %$1% or publisher like %$1%)  and deleted_at is null order by id desc limit 10 offset $2',[text,page])
        client.query(sql,[text,page])
        ]).then(function([result]) {    
                var data = {
                    "success":true,
                    "data":result.rows
